...
# All rights reserved.
#
# $Id: sdl.py,v 1.7 2018/05/25 09:47:32 ohsaki Exp ohsaki $
#
...
import cell
from cell.monitor.null import Null
...
class SDL(Null):
    ...
    def draw_line(self, sx, sy, dx, dy, width, color, alpha):
        ...
        xx = width / 2 * math.cos(cell.PI / 2 + theta)
        yy = width / 2 * math.sin(cell.PI / 2 + theta)
    ...
    def _render_bitmap(self, obj):
        if not obj._bitmap_cache:
            ...
            if not img:
                cell.die('pygame.image.load_extended(%s) failed.', file)
        ...
        if not obj._text_cache:
            if not self.font_cache.get(obj.size, None):
                font = pygame.font.Font(
                    '/usr/share/fonts/TTF/VL-Gothic-Regular.ttf', obj.size)
                if not font:
                    cell.die('pygame.font.SysFont() failed.')
                self.font_cache[obj.size] = font

            font = self.font_cache[obj.size]
            width, height = 0, 0
            color = self.rgba('lilywhite')
    ...
    def _render_wire(self, obj):
        ...
        xsign = cell.cmp(x2 - x, 0)
        ysign = cell.cmp(y2 - y, 0)
        delta = obj.width / 2
        xmid = (x + x2) / 2
